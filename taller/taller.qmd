...
```{r}
#| include: false
pacman::p_load(dplyr, ggplot2, sjlabelled, sjmisc, sjPlot)
...
::: {.fragment}
![](imagenes/cora1.png){.centro style="width: 800px;"}
:::
...
::: {.fragment}
![](imagenes/pipe_shortcut_mac.png){.centro style="width: 500px;"}
:::

::: {.fragment}
![](imagenes/pipe_shortcut_win.png){.centro style="width: 500px;"}
:::
...
::: {.fragment .pad}
En nuestra encuesta, variables como `ideologia` ya vienen como factor: `class(datos$ideologia)`.
:::
...
::: {.fragment .pad}
Al asignar `b <- a`, `b` se queda con una **copia** del valor de `a` en ese momento. Cambiar `a` después no cambia `b`: por eso `a` termina en 4, `b` en 5, y `a + 10` da 14.
:::
...
# Herramientas básicas de procesamiento de datos {background-color="#523870" .texto-blanco}
...
::: {.fragment .pad}
Hoy vamos a usar `{dplyr}` y `{ggplot2}` para procesar y graficar, y `{sjlabelled}`, `{sjmisc}`, y `{sjPlot}`, pensados especialmente para datos de encuestas.
:::
...
::: {.fragment .pad}
Antes de seguir, conozcamos `count()`: cuenta cuántos casos hay por cada valor único de una o más variables.
```{r}
datos |> 
  count(sexo) # cuántos casos hay por cada categoría de sexo
```
:::
...
datos |>
  select(fichas_salud) |>
  mutate(prioriza_salud = if_else(fichas_salud >= 5, 1, 0)) |>
  head()
```
...
datos |>
  select(fichas_salud) |>
  mutate(prioriza_salud = if_else(fichas_salud >= 5, "Sí", "No")) |>
  head()
```
:::
...
sjmisc::descr(datos_proc, fichas_salud)
```
...
```{r}
ggplot(datos_proc, aes(x = fichas_salud)) +
...
```{r}
ggplot(datos_proc, aes(x = ideologia, y = fichas_seguridad)) +
  geom_boxplot() +
